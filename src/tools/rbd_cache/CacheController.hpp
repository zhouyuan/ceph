...
class CacheController {
 public:
  ...
  int init() {
    thread_pool_singleton = &m_cct->lookup_or_create_singleton_object<ThreadPoolSingleton>(
    ...
    int r = m_object_cache_store->init(0);
    if (r < 0) {
  ...
  void handle_request(uint64_t sesstion_id, std::string msg){
    ...
    switch (io_ctx->type) {
      ...
      case RBDSC_READ: {
        ...
        m_object_cache_store->lookup_object(io_ctx->pool_name, io_ctx->vol_name);
