...
public: 
  SimplePolicy(uint64_t _block_num, float _level) 
    : m_level(_level),
      m_lock("SimplePolicy"),
      m_entry_count(_block_num)
  {
  ...
  int evict_object(std::string& out_cache_file_name) {
    ...
    // if this entry is in use, just touch it and avoid to evict it.
  ...
  // 1: when delete file, delete corresponding metadata
  //
  // delete metadata of l
  /*
  int invalidate(std::string _file_name) {
    Mutex::Locker locker(m_lock);
  ...
  */
  ...
  void update_status(std::string _file_name, CACHESTATUS _status) {
    ...
    // this object is in use, forbid evicting it. 
    if(entry->status == PROMOTED && _status == USING) {
      entry->status = USING;
      return;
    }

    // this object is absent for using 
    if(entry->status == USING && _status == PROMOTED) {
      entry->status = PROMOTED;
      return;
    }
...
private:
  ...
  //static const int m_entry_count = 4096; // TODO
  Entry m_entries[4096]; // TODO
