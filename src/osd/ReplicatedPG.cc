...
void ReplicatedPG::do_op(OpRequestRef op)
{
  ...
  if (hit_set) {
	...
	hit_set_start_stamp + pool.info.bloom_period <= m->get_recv_stamp()) {
      hit_set_persist();
    }
  }
...
{
  if (!pool.info.bloom_count || !pool.info.bloom_period) {
    hit_set_clear();
    return;
  }
...
void ReplicatedPG::hit_set_create()
{
  ...
  double actual_fpp = (double)pool.info.bloom_fpp_micro / 1000000.0;
  double bin_fpp = actual_fpp / pool.info.bloom_count;

  // oh yay, estimate target size based on the previous bin!
  if (hit_set_start_stats) {
    /*
    ...
    */
    ...
    target_size = (double)unique * (double)pool.info.bloom_period / (double)dur;
  }
  if (target_size < 100)
    target_size = 100;
  ...
  dout(10) << __func__ << " target_size " << target_size
	   << " fpp " << bin_fpp << dendl;
  hit_set.reset(new HitSet(new BloomHitSet(target_size, bin_fpp, now.sec())));
