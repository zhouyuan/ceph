...
Context *OpenRequest<I>::send_init_image_cache(int *result) {
  ...
  if (m_image_ctx->old_format || (m_image_ctx->read_only && !m_image_ctx->child) ||
      !m_image_ctx->shared_cache_enabled) {
    *result = 0;
    return m_on_finish;
  }
  ...
  // TODO: hard-coded SharedImageCache for prototype
  m_image_ctx->image_cache = new cache::SharedImageCache<ImageCtx>(*m_image_ctx);
  Context *ctx = create_context_callback<
