...
void SharedPersistentObjectCacherObjectDispatch<I>::init() {
  ...
  if (m_image_ctx->parent != nullptr) {
    ldout(cct, 5) << "child image: skipping cache client" << dendl;
    return;
  }

  ldout(cct, 5) << "parent image: setup cache client = "
                 << m_image_ctx->name
                 << dendl;
  ...
  if (ret < 0) {
    ldout(cct, 5) << "cache client fail to connect: " << dendl;
  } else {
    ldout(cct, 5) << "cache client register volume "
                   << "name = " << m_image_ctx->id 
                   << dendl;
...
bool SharedPersistentObjectCacherObjectDispatch<I>::read(
  ...
  if (m_cache_client && m_cache_client->connected && m_object_store) {
    ...
    ldout(cct, 20) << "object exists:" << exists << dendl;
    if (exists) {
      ldout(cct, 20) << "XXXX: " << read_data->length() << dendl;
  ...
  ldout(cct, 20) << "continue read" << dendl;
...
void SharedPersistentObjectCacherObjectDispatch<I>::client_handle_request(std::string msg) {
  ...
  switch (io_ctx->type) {
    ...
    case RBDSC_READ_REPLY: {
      ldout(cct, 20) << "client start to read cache" << dendl;
      //TODO(): should call read here

      break;
    }
    case RBDSC_READ_RADOS: {
      ldout(cct, 20) << "client start to read rados" << dendl;
      //TODO(): should call read here
