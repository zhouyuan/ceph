...
int ObjectCacheStore::lookup_object(std::string pool_name, std::string object_name) {
  ...
  if(m_ioctxs.find(pool_name) == m_ioctxs.end()) {
    ...
    ret = m_rados->ioctx_create(pool_name, *io_ctx);
    if(ret < 0) {
  ...
  int object_size = 4096; // fix size ??? TODO
  ...
  // persistent into ssd.
  CacheStore::SyncFile cache_file(m_cct, m_work_queue, cache_file_name);
  cache_file.open();
...
void ObjectCacheStore::shutdown() {
  m_rados->shut_down();
}
