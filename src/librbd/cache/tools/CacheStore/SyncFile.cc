...
SyncFile::SyncFile(CephContext *cct, ContextWQ &work_queue, const std::string &name)
...
int write_object_to_file(librados::bufferlist &read_buf, uint64_t object_len) {
  int ret;

  // buferlist --> c_str
  // TODO
  ret = std::pwrite(m_fd, read_buf.c_str(), object_len); 
  if(ret < 0) {
