...
// later, will modify sync file...sdh
int SyncFile::create_file() { 
  m_fd = ::open(m_name.c_str(), O_CREAT | O_RDWR | O_SYNC | O_DSYNC, S_IRWXU | S_IRWXO);
  if(m_fd == -1) { 
    lderr(cct) << "create file fails : " << std::strerror(errno) << dendl;
    return -1;
  }
  return m_fd;
}
