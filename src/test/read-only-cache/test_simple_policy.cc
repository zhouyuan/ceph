...
#include "tools/ceph_immutable_object_cache/SimplePolicy.hpp"
...
public:    
  ...
  uint64_t m_entry_index;     // just for generate only one file name
  ...
  void SetUp() override {
    m_simple_policy = new SimplePolicy(m_entry_num, 0.1);
  ...
  void insert_entry_into_promoted_lru(std::string cache_file_name) {
    ...
    ASSERT_EQ(OBJ_CACHE_PROMOTING, m_simple_policy->get_status(cache_file_name));
...
TEST_F(TestSimplePolicy, test_lookup_miss_and_no_free) {
  ...
  ASSERT_TRUE(m_simple_policy->lookup_object("no_this_cache_file_name") == OBJ_CACHE_NONE);
} 

// have free cache space, and cache miss
TEST_F(TestSimplePolicy, test_lookup_miss_and_free_space) {
  ...
  ASSERT_TRUE(m_simple_policy->get_status("miss_but_have_free_space_file_name") == OBJ_CACHE_PROMOTING);
}
...
TEST_F(TestSimplePolicy, test_lookup_hit_and_promoting) {
  ...
  ASSERT_TRUE(m_simple_policy->get_status("promoting_file_1") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_file_2") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_file_3") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_file_4") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->lookup_object("promoting_file_1") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->lookup_object("promoting_file_2") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->lookup_object("promoting_file_3") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->lookup_object("promoting_file_4") == OBJ_CACHE_PROMOTING);
}
...
// update cache status : promoting --> none
TEST_F(TestSimplePolicy, test_update_state_from_promoting_to_none) {
  ...
  ASSERT_TRUE(m_simple_policy->get_promoting_entry_num() == 4);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_to_none_file_1") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_to_none_file_2") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_to_none_file_3") == OBJ_CACHE_PROMOTING);
  ASSERT_TRUE(m_simple_policy->get_status("promoting_to_none_file_4") == OBJ_CACHE_PROMOTING);
...
// update cache status : promoted --> none
...
// update cache status : promoting --> promoted
...
// evict list : don't need to evict
...
// evict list : need to evict ten entry.
